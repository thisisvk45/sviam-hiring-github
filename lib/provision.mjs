...
export function auditAccess(api, repo, expectedLogins) {
  const metadata = api("GET", `repos/${repo}`);
  if (!metadata.private || metadata.visibility !== "private" || metadata.fork) throw new Error("Repository must be an independent private repository.");
...
export function provision(api, rawOptions) {
  ...
  if (!source.is_template || !source.private) throw new Error("Use the staff-controlled private starter template.");
  if (source.owner.login.toLowerCase() !== owner.toLowerCase()) throw new Error("Template must belong to the selected owner.");
  auditAccess(api, template, [owner]);
